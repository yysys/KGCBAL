...
public class TestRun {
    ...
    private static void prepare(int rate, int number) throws IOException {
        GlobalValueInit();
        ...
        File f = new File("resource/data/" + rate + "/test" + number + ".txt");
        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(f),"UTF-8"));
        String line;
        while ((line = reader.readLine()) != null) {
            String[] split_data = line.split("\t");
    ...
    public static void test_run(int rate, int number) throws IOException {
        test = new Test();
        prepare(rate, number);
        test.run();
    }
