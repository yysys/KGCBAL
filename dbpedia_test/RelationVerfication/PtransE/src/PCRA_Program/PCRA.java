...
public class PCRA {
    ...
    private void prepare(int rate) throws IOException {
        relation_num = Read_Data("resource/data/"+ rate + "/relation2id.txt", relation2id, id2relation);

        File f = new File("resource/data/" + rate + "/train.txt");
        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(f),"UTF-8"));
        String line;
        while ((line = reader.readLine()) != null) {
            String[] split_data = line.split("\t");
    ...
    public void run(int rate) throws IOException {
        init();
        prepare(rate);
        ...
        Write_Path();
        Write_Confident();
        calculate_prob("train", rate);
        calculate_prob("test", rate);
    }
    ...
    private void calculate_prob(String file_name, int rate) throws IOException {
        File f = new File(String.format("resource/data/" + rate + "/%s.txt", file_name));
        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(f),"UTF-8"));
